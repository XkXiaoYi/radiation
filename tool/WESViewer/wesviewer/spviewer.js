...
function draw_graph_bqmon_spectrum(file, file_comment, data) {
	...
	var sp_info_mapping = [
		["SampleInfo Name",             "名称",    ""],
		["SampleInfo Location",        "采集地点", ""],
		["SampleInfo Time",            "采集时间", ""],
		["SampleInfo Weight",          "样本重量",     "kg"],
		["SampleInfo Volume",          "样本数据量",     "l"],
		["SampleInfo Note",            "备注",     ""],
		["DeviceConfigReference Name", "设备配置名称", ""],
		["StartTime",                  "测量开始时间", ""],
		["EndTime",                    "测量结束时间", ""]
	];
...
function setup_multiple_spectrum(spectrum) {
	...
	var html = '■光谱选择<br><table class="csv">';
...
function save_tsv(spectrum) {
	var html = '';
	html += 'TSV数据：测量时间: ' + spectrum['time'] + ' 秒 ';
	html += '<input type="button" id="tsv_data_close" value="关闭"><br>';
...
function print_sp_info_compare(spectrums) {
	...
	html += '*光谱文件的详细信息只显示在样本本体（非BG）上。<br>';
